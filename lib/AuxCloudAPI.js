...
class AuxCloudAPI {
  ...
  async login(email, password) {
    ...
    if (result.status === 0) {
      ...
      this.license = result.license || LICENSE;
  ...
  async getDevices(familyid, shared = false) {
    ...
    if (result.status === 0) {
      ...
      // Implement correct fetching strategy from HA integration:
      // 1. Fetch standard parameters (using empty list [])
      // 2. Fetch special parameters (using ['mode'])
      ...
      for (const device of devices) {
        if (device.state === 1) { // Only query online devices
          try {
            ...
            let standardParams = {};
            try {
              standardParams = await this.getDeviceParams(device, []);
            } catch (err) {
              console.error(`Failed to get standard params for device ${device.endpointId}:`, err);
            }

            // 2. Special parameters (including envtemp via 'mode')
            let specialParams = {};
            try {
              specialParams = await this.getDeviceParams(device, ['mode']);
            } catch (err) {
              console.error(`Failed to get special params for device ${device.endpointId}:`, err);
            }

            // 3. Merge
            device.params = { ...standardParams, ...specialParams };
          } catch (error) {
            console.error(`Critical error updating params for device ${device.endpointId}:`, error);
            device.params = {};
          }
        } else {
          device.params = {};
  ...
  async _getDeviceParamsInternal(device, paramsList = []) {
    ...
    const data = {
      directive: {
        ...
        payload: {
          act: 'get',
          params: paramsList,
          vals: paramsList.map(() => [{ val: 0, idx: 1 }]), // Match structure from Python SDK special case
          did: device.endpointId
        }
      }
    };
    ...
    let endpointUrl = 'device/control/v2/sdkcontrol';
    if (this.license) {
      endpointUrl += `?license=${this.license}`;
    }

    const result = await this._makeRequest('POST', endpointUrl, {
      data,
      headers: {
        'Content-Type': 'application/json'
      }
