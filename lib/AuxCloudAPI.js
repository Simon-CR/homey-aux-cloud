...
class AuxCloudAPI {
  ...
  async getDevices(familyid, shared = false) {
    ...
    if (result.status === 0) {
      ...
      // Testing if specific param request works AT ALL by requesting likely supported 'pwr'
      for (const device of devices) {
        if (device.state === 1) { // Only query online devices
          try {
            const params = await this.getDeviceParams(device, ['pwr']);
  ...
  async _getDeviceParamsInternal(device, paramsList = []) {
    ...
    const data = {
      directive: {
        ...
        // Only include vals if not empty (some servers reject empty vals array)
        if(paramsList.length > 0 && paramsList[0] === 'set_vals_placeholder') {
          // logic if we were setting
        } else {
          // For get, try omitting vals completely? 
          // Or maybe it needs to be explicitly null?
          // The user quoted payload HAD vals: []
          // Let's try matching the user's quoted payload EXACTLY?
          // The user quoted: 'vals': [] 
          // I was sending vals: []

          // Wait, let's try removing it just in case
        }

    // Let's try sending vals: [] again but verify the exact text in debug
    data.directive.endpoint.payload.vals = [];
  }
};
...
if (paramsList.length > 0) {
  console.log('[DEBUG] Params requested:', JSON.stringify(paramsList));
