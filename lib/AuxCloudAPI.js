...
class AuxCloudAPI {
  ...
  async getDevices(familyid, shared = false) {
    ...
    if (result.status === 0) {
      ...
      // Request comprehensive list including envtemp (ambient temperature)
      // Empty array returns default set which doesn't include envtemp
      const comprehensiveParams = [
        'pwr', 'temp', 'envtemp', 'ac_mode', 'ac_mark', 'ac_vdir', 'ac_hdir',
        'ac_slp', 'ac_astheat', 'ecomode', 'ac_clean', 'ac_health', 'scrdisp',
        'mldprf', 'pwrlimit', 'pwrlimitswitch', 'childlock', 'comfwind',
        'sleepdiy', 'ac_tempconvert', 'tempunit', 'err_flag'
      ];
