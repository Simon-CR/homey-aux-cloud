...
class AuxCloudAPI {
  ...
  async getDevices(familyid, shared = false) {
    ...
    if (result.status === 0) {
      ...
      // Retrying comprehensive list (User suggestion) with DEBUG LOGGING
      // The previous attempt failed silently, now we will see why
      const comprehensiveParams = [
        'ac_astheat', 'ac_clean', 'ac_hdir', 'ac_health', 'ac_mark', 'ac_mode',
        'ac_slp', 'ac_vdir', 'ecomode', 'err_flag', 'mldprf', 'pwr', 'scrdisp',
        'temp', 'envtemp', 'pwrlimit', 'pwrlimitswitch', 'childlock', 'comfwind',
        'sleepdiy', 'ac_tempconvert', 'tempunit'
      ];

      for (const device of devices) {
        if (device.state === 1) { // Only query online devices
          try {
            const params = await this.getDeviceParams(device, comprehensiveParams);
