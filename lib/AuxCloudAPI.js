    ...
    const data = {
      directive: {
        ...
        payload: {
          act: 'get',
          params: paramsList,
          vals: [], // Default to empty array as per HA notebook
          did: device.endpointId
        }
      }
    };

    // Experiment: Remove vals key if trying to fetch info? 
    // The user suggested 'vals': [] so keeping it [] is likely correct.
    // However, my previous comprehensive params attempt might have failed due to syntax error in my test?
    // Let's ensure structure is perfect.
