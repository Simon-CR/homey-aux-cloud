...
class AuxCloudAPI {
  ...
  async _getDeviceParamsInternal(device, paramsList = []) {
    ...
    const data = {
      directive: {
        ...
        payload: {
          act: 'get',
          params: paramsList,
          vals: new Array(paramsList.length).fill(null), // Try matching length with nulls
          did: device.endpointId
        }
      }
    };
    ...
    if (paramsList.length > 0) {
      ...
      console.log('[DEBUG] Vals sent:', JSON.stringify(data.directive.endpoint.payload.vals));
    }
