...
class AuxACDevice extends Homey.Device {
  ...
  async onInit() {
    ...
    try {
      ...
      // Log doesn't exist yet - it will be created automatically when we first use createEntry
      this.log('Insights log will be created on first energy data entry');
      this.meterPowerLog = null; // Will be initialized on first data
    }
